...
class Order(BaseModel):
    ...
    status = models.CharField(max_length=2, choices=OrderStatus.choices, default=OrderStatus.CREATED)

    def __str__(self):
        return self.no

class OrderLine(BaseModel):
    order = models.ForeignKey(Order, related_name='order_lines', on_delete=models.RESTRICT)
...
class ShippingAddress(BaseModel):
    order = models.OneToOneField(Order, related_name='shipping_address', on_delete=models.RESTRICT)
